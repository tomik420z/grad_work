...
protected:
    ...
    std::vector<fractional_num> vec_delta; // массив дельт  
    ...
    void convert_line(std::vector<fractional_num>& line, size_t __i, size_t __j) {
        const fractional_num frac = line[__j];
    ...
    void substract(const std::vector<fractional_num>& main_line, std::vector<fractional_num>& sub_line, coeff_table_t::member_iterator __iter, size_t __i, size_t __j) {
        const fractional_num factor = sub_line[__j];
    ...
    void gauss_step(std::vector<fractional_num>& main_line, size_t __i, size_t __j) {
        convert_line(main_line, __i, __j);
        //std::cout << "+\n";
        auto it = table.free_begin();
        for(auto& line : table) {
            if (&main_line != &line && !line[__j].check_zero()) {
    ...
    void __calc_delta() {
        ...
        for(size_t i = 0; i < count_of_line; ++i) {
            vec_delta.back() += table.get_free_members()[i] * obj_func_coeff[vec_name[i]];
        }
    }
    ...
    size_t find_permission_line(size_t j) {
        ...
        fractional_num val = std::numeric_limits<int>::max();
        for(size_t i = 0; i < table.size(); ++i) {
            fractional_num ratio = table[i][j].check_more_zero() ? ref_free_member[i] / table[i][j] : std::numeric_limits<int>::max(); 
            if (ratio < val) {
                val = ratio;
                index = i;
            }
    ...
    bool check_for_integer() const noexcept {
        ...
        for(size_t i = 0; i < line_free.size(); ++i) {
            ...
            if(static_cast<size_t>(vec_name[i]) < __count_column) {
                if (!line_free[i].get_frac_part().check_zero()) { // решение не целочисленное 
                    return false;
                }
            } 
        } 
    ...
    */
    ...
    size_t find_max_fractional() const noexcept {
        ...
        fractional_num max_val = std::numeric_limits<int>::min();
        for(size_t i = 0; i < line_free.size(); ++i) {
            // проверяем дробную часть
            
            if (fractional_num frac = line_free[i].get_frac_part(); 
    ...
    size_t find_premmision_column(size_t i) {
        ...
        fractional_num min_val = std::numeric_limits<int>::max();
        size_t index_res = 0;
        for(size_t i = 0; i < last_line.size(); ++i) {
            if (!last_line[i].check_zero()) {
                if (fractional_num del = vec_delta[i] / last_line[i]; del < min_val) {
                    min_val = del;
                    index_res = i;
                }
            } 
...
public:
    ...
    while(true) {
        ...
        for (auto iter = f_max(); true; iter = f_max()) {
            ...
            //std::cout << *iter << std::endl;
            if ((*iter).check_less_zero() || (*iter).check_zero()) {
                break;
            }
        ...
        // std::cout << i << " " << j << std::endl; 
            ...
            std::vector<fractional_num> integer_restriction; //ограничение для целочисленности
            ...
            std::transform(std::cbegin(table[index_max_frac]), std::cend(table[index_max_frac]), std::begin(integer_restriction), [](const auto& val){
                if (!val.check_integer()) { // если число дробное  
                    if(val.check_less_zero()) {
                        return -(val - fractional_num::floor(val));
                    } else {
                        return -(val.get_frac_part());
                    }
                }
                return fractional_num{};
            });

            fractional_num b = -table.get_free_members()[index_max_frac].get_frac_part();
    ...
    */        
    ...
    std::vector<std::pair<size_t, size_t>> get_path() const {
        ...
        for(size_t i = 0; i < vec_name.size(); ++i) {
            if (!free_line[i].check_zero() && (vec_name[i] < static_cast<int>(count_variables))) {
