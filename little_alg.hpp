...
public:
    ...
    bool row_without_inf(matrix_t & mx, typename matrix_t::row_iterator r) {
        auto row_end = mx.row_end();
    ...
    typename matrix_t::col_iterator get_rib_without_inf(matrix_t &mx) {
        auto p = mx.head_col();
        auto row_end = mx.row_end();
        auto col_end = mx.col_end();
    ...
    void add_the_last_two_edges(way_t & way, matrix_t & M, bool &bad_dist) {
        ...
        if (*pt00 < infinity_2) {
            way.push_back(M.get_edge(pt00));
            way.push_back(M.get_edge(pt11));
        } else {
            way.push_back(M.get_edge(pt01));   
            way.push_back(M.get_edge(pt10)); 
        }
    }
    ...
    void recoursive_procedure(ptr_matrix_t&& M1, ptr_way_t &&way, value_type border_lim) {
       ...
       // std::cout << "-------------------------------\n"; 
        ...
        if (M1->size() > 2) {
            ...
            long long sum = calc_distance(*way);
        ...
        if (M2.size() > 2) {
            ...
            bool fl = true;
            add_the_last_two_edges(copy_way, M2, fl);
    ...
    void operator()() {
        ...
        //mx_temp.print();
        if (ptr_mx->size() == 1) {
            min_way.push_back(ptr_mx->get_edge(ptr_mx->col_begin()));   
            record = 0;
            return;
